...
class PlaydateDetailViewModel: ObservableObject {
        ...
        query1.getDocuments { [weak self] snapshot1, error1 in
            ...
            if let documents = snapshot1?.documents {
                for doc in documents {
                    if let data = doc.data() as? [String: Any], let friendId = data["friendID"] as? String {
            ...
            query2.getDocuments { [weak self] snapshot2, error2 in
                ...
                if let documents = snapshot2?.documents {
                    for doc in documents {
                        if let data = doc.data() as? [String: Any], let friendId = data["userID"] as? String {
                            friendIds.append(friendId)
                        }
                    }
                }
    ...
    func joinPlaydate(playdateId: String, userId: String) {
        ...
        db.runTransaction({ (transaction, errorPointer) -> Any? in
            ...
            guard let data = playdateDocument.data() as? [String: Any] else {
            ...
            if let playdateId = playdateId, let self = self {
                ...
                db.collection("users").document(userId).getDocument { [weak self] snapshot, error in
                    ...
                    if let snapshot = snapshot, snapshot.exists {
                        do {
                            ...
                            print("Error decoding user for comment: \(error.localizedDescription)")
                            completion(false)
                        }
                    } else {
                        completion(false)
