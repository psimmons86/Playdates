...
echo -e "${BLUE}===========================================================${NC}"
echo -e "${BLUE}         Verifying Critical Files for Compilation          ${NC}"
echo -e "${BLUE}===========================================================${NC}"
...
for file in "${FILES_TO_CHECK[@]}"; do
  ...
  # Copy file to temp directory to isolate it
  cp "$file" "$TEMP_DIR/$(basename "$file")"
  
  # Check syntax using swiftc -parse instead of swift -syntax-only
